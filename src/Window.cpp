...
D3D12_CPU_DESCRIPTOR_HANDLE Window::GetFirstPassRenderTargetView() const
{
	CD3DX12_CPU_DESCRIPTOR_HANDLE firstPassRtvHandle = CD3DX12_CPU_DESCRIPTOR_HANDLE(
		m_firstPassRTVDescriptorHeap->GetCPUDescriptorHandleForHeapStart(),
		m_CurrentBackBufferIndex * FirstPassRTVCount, m_RTVDescriptorSize);

	return firstPassRtvHandle;
}
